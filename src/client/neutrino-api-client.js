...
const APIErrorCode = require('../client/api-error-code');
const APIResponse = require('../client/api-response');
    ...
    execRequest(httpMethod, endpoint, params, outputFilePath, timeoutInSeconds) {
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                this.client.setTimeout(this.CONNECT_TIMEOUT);

                this.client.on('timeout', () => {
                    if (this.client.connecting) {
                        const apiResponse = APIResponse.ofErrorCode(statusCode, contentType, APIErrorCode.CONNECT_TIMEOUT);
                        reject(apiResponse)
                        this.destruct();
                    }
                })
                ...
                this.client.on('error', (error) => {
                    switch (error.code) {
                        case 'ECONNREFUSED':
                            apiResponse = APIResponse.ofErrorCode(statusCode, contentType, APIErrorCode.CONNECT_TIMEOUT);
                            break;
                        default:
                    ...
                    if (error.message.includes('ssl3_get_record:wrong version number')) {
                        apiResponse = APIResponse.ofErrorCode(statusCode, contentType, APIErrorCode.TLS_PROTOCOL_ERROR);
                    }
                    reject(apiResponse);
                });
                ...
                if (httpMethod === 'GET') {
                    ...
                    httpMethodConst = http2.constants.HTTP2_METHOD_GET;
                } else {
                    httpMethodConst = http2.constants.HTTP2_METHOD_POST;
                }
                request = this.client.request({
                    [http2.constants.HTTP2_HEADER_METHOD]: httpMethodConst,
                    [http2.constants.HTTP2_HEADER_PATH]: `/${endpoint}${query}`,
                    'User-id': this.userID,
                    'API-Key': this.apiKey,
                ...
                request.setTimeout(timeoutInSeconds * 1000, () => {
                    apiResponse = APIResponse.ofErrorCode(statusCode, contentType, APIErrorCode.READ_TIMEOUT);
                    reject(apiResponse);
                    request.destroy();
                    this.destruct();
                });
                ...
                request.on('error', (_error) => {
                    // Prevent "Unhandled 'error' event" here and intentionally do nothing.
                    // Cause of error is handled by client.on('error', ... )
                });
                ...
                switch (error.code) {
                    case 'ERR_HTTP2_UNSUPPORTED_PROTOCOL':
                        apiResponse = APIResponse.ofErrorCode(statusCode, contentType, APIErrorCode.URL_PARSING_ERROR);
                        break;
                    default:
                        apiResponse = APIResponse.ofErrorCode(statusCode, contentType, APIErrorCode.NETWORK_IO_ERROR);
                        break;
                }
                reject(apiResponse);
            }
